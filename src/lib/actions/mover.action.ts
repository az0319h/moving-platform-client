...
console.log('API_BASE URL:', process.env.NEXT_PUBLIC_BASE_URL);

const API_BASE = `${process.env.NEXT_PUBLIC_BASE_URL}/movers` || 'http://localhost:4000/movers';
...
// 필터링 및 정렬 함수
const filterAndSortMovers = (movers: Mover[], params: GetMoversParams): Mover[] => {
  let filtered = [...movers];
  ...
  if (params.area && params.area !== 'all') {
    ...
    filtered = filtered.filter(mover => {
      const hasRegion = mover.region.some(region => targetRegions.includes(region));
      console.log(`${mover.nickName} - regions: ${mover.region.join(', ')} - match: ${hasRegion}`);
      return hasRegion;
    });
...
export const getMovers = async (params: GetMoversParams = {}, token?: string): Promise<GetMoversResponse> => {
  try {
    const { page = 1, limit = 10, search, area, serviceType, sortBy } = params;
    ...
    const res = await axios.get(`${API_BASE}?${queryParams.toString()}`, {
    ...
    const generateConsistentData = (moverId: string) => {
      // ID를 숫자로 변환해서 시드로 사용
      const seed = moverId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      ...
      return {
        movers: data.movers.map(mover => {
          ...
          return {
            ...
            averageReviewRating: mover.averageReviewRating || generatedData.averageReviewRating,
            reviewCount: mover.reviewCount || generatedData.reviewCount,
            estimateCount: mover.estimateCount || generatedData.estimateCount,
          };
        }) as Mover[],
    ...
    for (let i = 0; i < 3; i++) { // 3번 반복하여 30개의 데이터 생성
      MOCK_MOVERS.forEach((mover, index) => {
        expandedMovers.push({
          ...mover,
...
export const getMoverById = async (id: string, token?: string): Promise<Mover> => {
  const res = await axios.get<Mover>(`${API_BASE}/${id}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  return res.data;
};
